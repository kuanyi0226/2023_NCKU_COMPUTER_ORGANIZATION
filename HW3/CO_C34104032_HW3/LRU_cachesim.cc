...
void cache_sim_t::init()
{
  ...
  for(int i = 0; i < 8; i++){
    for(int j = 0; j <8; j++){
      lru_time[i][j] = j; //init the time priority in order
    }
  }
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
      //hit
      //1.others blocks higher their priorities to be replaced(older time)
      //if the time value is bigger(newer) than current hit block
      for(int j = 0; j < ways; j++){
        if(lru_time[idx][j] > lru_time[idx][i]){
          lru_time[idx][j] -= 1;
        }
      }
      //2.update the latest time for currently hit block
      int resetValue = ways -1; //latest time
      lru_time[idx][i] = resetValue; //set the hit block to latest time 
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  for(int i = 0; i < ways; i++){
    if(lru_time[idx][i] == 0){ 
      ...
      int resetValue = ways -1; //latest time
      lru_time[idx][i] = resetValue;
      //other blocks minus one (getting "older")
      for(int j = 0; j < ways; j++){
        if(j != i){
          lru_time[idx][j] -= 1;
        }

...
{
  size_t way = 0;
  size_t temp_tag = (addr >> idx_shift) | VALID;
  size_t idx = (addr >> idx_shift) & (sets - 1);
  
  if(fifo[idx].size() >= ways){ //the set is full: replace
    size_t old_way = fifo[idx].front().first;
    fifo[idx].erase(fifo[idx].begin());
    fifo[idx].push_back(make_pair(old_way,temp_tag));
    way = old_way;
  }else{ //the set is not full
    way = fifo[idx].size();
    fifo[idx].push_back(make_pair(fifo[idx].size(),temp_tag));  
  }
